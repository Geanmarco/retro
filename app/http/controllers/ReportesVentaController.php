<?php
    ...
    ";
		...
		public function comprobanteCotizacion($coti) {
            ...
            join productos p on p.id_producto = pc.id_producto where pc.id_coti='$coti' AND codigo = 'SERVICIO'
            UNION
            SELECT pc.*,p.descripcion,p.codigo from productos_cotis pc
            join productos p on p.id_producto = pc.id_producto where pc.id_coti='$coti' AND descripcion like 'FIL%'
            UNION

            SELECT pc.*,p.descripcion,p.codigo from productos_cotis pc
            join productos p on p.id_producto = pc.id_producto where pc.id_coti='$coti' AND descripcion like 'acei%'
            UNION
        ...
        <td class='' style=' font-size: 11px; text-align: center;border-left: 1px solid #363636;border-right: 1px solid #363636;'> </td>
			...
			$this->mpdf->WriteFixedPosHTML("<span style=' font-size: 12px'><strong>Email: </strong> ventas@retrotracsys.com</span>", 15, 32, 210, 130);
        ...
        <table style='width:100%;border-bottom: 1px solid #363636;border-collapse: collapse;'>
         ...
         <br>
           <td style='font-size: 11px'><strong>NÙMERO DE CUENTA BCP</strong></td>
           <br>Cuenta Corriente: 3809967102079</br>
           <br> CCI: 002-380-009967102079-42</br>
  <br>
           <td style='font-size: 11px'><strong>NÙMERO DE CUENTA BBVA</strong></td>
           <br>Cuenta Corriente: 0011-0053-0100022221</br>
           <br> CCI: 0011-0053-0100022221-64</br>
      ...
      where ne.nota_id =" . $venta;
		...
		public function guiaRemision($guia, $nombreXML) {
			...
			$this->mpdf->WriteFixedPosHTML("<img style='max-width: 300px;max-height: 85px' src='" . URL::to('files/logos/' . $datoEmpresa['logo']) . "'>", 15, 5, 150, 120);
			$this->mpdf->WriteFixedPosHTML($htmlCuadroHead, 0, 5, 195, 130);
			$this->mpdf->WriteFixedPosHTML("<span style=' font-size: 12px;margin: 1pt 2pt 3pt;'><strong>Central Telefónica: </strong> +51987891373</span>", 15, 27, 210, 130);
			$this->mpdf->WriteFixedPosHTML("<span style=' font-size: 12px;margin: 1pt 2pt 3pt;'><strong>Email: </strong> ventas@retrotracsys.com</span>", 15, 32, 210, 130);
        ...
        <td class='' style=' font-size: 10px; text-align: center;border-left: 1px solid #363636;border-right: 1px solid #363636;'> </td>
			...
			$this->mpdf->WriteFixedPosHTML("<span style=' font-size: 12px'><strong>Email: </strong> ventas@retrotracsys.com</span>", 15, 40, 210, 130);
    ...
    join productos p on p.id_producto = productos_ventas.id_producto WHERE id_venta=" . $venta);
        ...
        <td class='' style=' font-size: 11px; text-align: center;border-left: 1px solid #363636;border-right: 1px solid #363636;'> </td>
			...
			$this->mpdf->WriteFixedPosHTML("<span style=' font-size: 12px'><strong>Email: </strong> ventas@retrotracsys.com</span>", 15, 46, 210, 130);
			...
			$dias = intval($datoVenta['dias_pagos']);
						...
						<div style='width: 100%;border: 1px solid black'>
							<div style='width: 55%; float: left; '>        
								<table style='width:100%'>
								  ...
								  <tr>
								   ...
								   <tr>
									<td style=' font-size: 10px;text-align: left'><strong>ORDEN DE COMPRA:</strong></td>
									<td style=' font-size: 10px;'>{$datoVenta['num_oc']}</td>
								  </tr>
								  <tr>
							...
							<div style='width: 45%; float: left'>
								<table style='width:100%'>
								  ...
								  <tr>
									<td style=' font-size: 11px;text-align: left'><strong>PAGO:</strong></td>
									<td style=' font-size: 11px;'>$tipo_pagoC$dias</td>
								  </tr>
								</table>
							</div>
							...
							<div style='text-align: center'>
								<span style='font-size: 11px;'> $pagoData</span>
							</div>
						</div>
        			</div>
        $tabla_cuotas
...
<div style="width: 100%;">
        <table style="width:90%;margin:auto;display: block;text-align:center;font-size: 10px;">
			...
			$formatter = new NumeroALetras;
			$totalLetras = $formatter->toInvoice(number_format($totalImporte, 2, '.', ''), 2, 'SOLES');
